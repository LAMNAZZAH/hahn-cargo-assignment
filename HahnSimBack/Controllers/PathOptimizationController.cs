...
namespace HahnSimBack.Controllers
{
    ...
    public class PathOptimizationController(AppDbContext context) : ControllerBase
    {
        private readonly AppDbContext _context = context;
        
        [HttpGet("find")]
        public async Task<ActionResult<ApiResponse<PathResult>>> FindShortestPath([FromQuery] int start, [FromQuery] int end)
        {
            try
            {
                ...
                if (path == null)
                {
                    return NotFound(ApiResponse<PathResult>.ErrorResponse(
                ...
                return Ok(ApiResponse<PathResult>.SuccessResponse(result, "Shortest path found successfully."));
            }
            catch (Exception ex)
            {
                return StatusCode(500, ApiResponse<PathResult>.ErrorResponse(
